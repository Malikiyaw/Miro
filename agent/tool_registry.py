...
TOOL_SPECS: Dict[str, Dict[str, Any]] = {
    ...
    "create_automation": _spec(
        "create_automation",
        "Create a LIVE automation that runs for real — 1000x scale. Types: scheduled_task (cron/schedule + channel + response/actions), event_trigger (event + filters + actions[] for member_joined/member_left/message_contains/reaction_added/voice_joined), auto_responder (keywords + response, single reply), reminder (duration sec 10-2592000 + response, one-shot), trigger_role (keywords + role_id/role_name + response). KEYWORDS are REQUIRED for event_trigger+message_contains, auto_responder, and trigger_role — pass as a JSON array of strings (e.g. ['hello','ping']) or a comma-separated string. MULTI-STEP: pass 'actions' as list of {name, parameters} to run several things in order (e.g. send_message then assign_role). Bulk: use bulk_create_automations for 25 at once (max 100/guild, auto-pause after 10 failures). Quota-aware: check list_automations first to avoid duplicates — update_automation if exists. Schedule: prefer schedule:{every_minutes:15}|{every_hours:2}|{every_days:1}|{daily_at:'09:00'}|{weekly_on:'Mon',at:'08:00'}|{weekly_on:['Mon','Fri']}|{cron:'0 12 * * *'}; plain words in cron ('daily at 9am','every 15 minutes','weekdays 8am','in 2 hours'->reminder) auto-parsed. Channel: 'here'/'current'/'this channel' resolved via query_channels or channel_name, else current channel. Use query_roles to resolve role_name→role_id. All fire live, survive restarts.",
        {"type": {"type": "string", "required": True, "description": "One of: scheduled_task, event_trigger, auto_responder, reminder, trigger_role."},
         "name": {"type": "string", "required": True},
         "response": {"type": "string", "required": False},
         "cron": {"type": "string", "required": False},
         "schedule": {"type": "object", "required": False},
         "duration": {"type": "integer", "required": False},
         "keywords": {"type": "array", "required": True, "description": "REQUIRED when type=event_trigger with event=message_contains, and for type=auto_responder / trigger_role. JSON array of strings (e.g. ['hello','ping']) or a comma-separated string."},
         ...
         "event": {"type": "string", "required": False, "description": "For type:event_trigger — one of member_joined, member_left, message_contains, reaction_added, voice_joined. message_contains requires 'keywords'."},
         "filters": {"type": "object", "required": False, "description": "Optional filters: channel_id, role_id, emoji. Can also carry 'keywords' for event_trigger."},
