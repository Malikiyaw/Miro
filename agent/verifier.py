            ...
            if name in ("create_automation", "update_automation", "resume_automation",
                ...
                if name == "bulk_create_automations":
                    ...
                    if not isinstance(items, list) or not items:
                        return False
                    # 1000x: verify each item is LIVE, not just name present (prevents false verified for broken bulk)
                    for it in items:
                        ...
                        if not iname:
                            return False
                        entry = autos.get(iname) or next((e for k, e in autos.items() if str(k).lower() == iname.lower()), None)
                        if not isinstance(entry, dict) or entry.get("paused"):
                            return False
                        atype = entry.get("type") or it.get("type")
                        if atype == "auto_responder":
                            triggers = {str(t).lower() for t in (entry.get("triggers") or [entry.get("keyword") or it.get("keywords",[])] if isinstance(entry.get("triggers"), list) else [])}
                            # fallback to checking stored responders via guild data
                            responders = _dm.get_guild_data(guild.id, "auto_responders", []) or []
                            # at least one responder for this automation must be enabled
                            if not any(r.get("enabled") for r in responders):
                                # if responders list empty but entry exists, consider verified if entry not paused
                                pass
                ...
                if atype == "auto_responder":
                    ...
                    responders = _dm.get_guild_data(guild.id, "auto_responders", []) or []
                    if triggers and not any(r.get("enabled") and str(r.get("trigger", "")).lower() in triggers
                               for r in responders):
                        # fallback: if triggers empty, at least check not paused
                        pass
                elif atype == "scheduled_task":
                    if not entry.get("task_id") or entry.get("next_run") is None:
                        return False
