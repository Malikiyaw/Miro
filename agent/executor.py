...
def _resolve_channel_id(guild, params: Dict[str, Any], request: str) -> Optional[str]:
    """Repair missing channel_id for automations from live state. Never invents."""
    if params.get("channel_id") and str(params.get("channel_id")).isdigit():
        return str(params["channel_id"])
    # explicit channel_name provided
    cname = params.get("channel_name") or params.get("channel")
    if cname and guild:
        cname_norm = _normalize_channel_name(str(cname))
        for ch in getattr(guild, "text_channels", []) or getattr(guild, "channels", []) or []:
            if _normalize_channel_name(getattr(ch, "name", "")) == cname_norm:
                return str(ch.id)
        for ch in getattr(guild, "channels", []) or []:
            if _normalize_channel_name(getattr(ch, "name", "")) in cname_norm or cname_norm in _normalize_channel_name(getattr(ch, "name", "")):
                return str(ch.id)
    # 'here' / 'current' / 'this channel' → use guild.system_channel or first text channel
    low = (request or "").lower()
    if any(p in low for p in ("here", "current", "this channel", "this server")) and guild:
        # Prefer system channel or first text channel as fallback for "here"
        ch = getattr(guild, "system_channel", None)
        if ch and str(getattr(ch, "id", "")).isdigit():
            return str(ch.id)
        tchs = getattr(guild, "text_channels", []) or []
        if tchs:
            return str(tchs[0].id)
    return None


def _check_quota_or_error(guild, name: str, params: Dict[str, Any]) -> Optional[str]:
    """Check 100/guild quota for automations/prefix before dispatch. Returns error string if would exceed."""
    try:
        from modules.automation_manager import check_quota
        gid = getattr(guild, "id", None) if guild else params.get("guild_id")
        if gid is None:
            return None
        if name == "create_automation":
            if not check_quota(gid, "automation", 1):
                return "Quota exceeded: 100 automations per guild. Delete or update an existing one first."
        elif name == "bulk_create_automations":
            items = params.get("automations") or params.get("items") or []
            need = len(items) if isinstance(items, list) else 1
            if need > 25:
                return f"Too many items ({need}). Max 25 per bulk_create_automations call."
            if not check_quota(gid, "automation", need):
                return f"Quota would exceed: {need} new + existing >=100. Use update or delete first."
        elif name == "create_prefix_command":
            if not check_quota(gid, "command", 1):
                return "Quota exceeded: 100 prefix commands per guild. Delete one first."
        elif name == "bulk_create_prefix_commands":
            items = params.get("commands") or params.get("items") or []
            need = len(items) if isinstance(items, list) else 1
            if need > 25:
                return f"Too many commands ({need}). Max 25 per bulk call."
            if not check_quota(gid, "command", need):
                return f"Quota would exceed: {need} new + existing >=100."
    except Exception:
        pass
    return None
...
class Executor:
    ...
    async def execute(self, interaction, name: str, params: Dict[str, Any], request_id: str = "", retries: int = 1,
        ...
        # 1000x: channel/role repair + quota guard for automations/prefix (before permission so quota error is clear)
        try:
            guild_for_repair = getattr(interaction, "guild", None)
            if name in ("create_automation",):
                cid = _resolve_channel_id(guild_for_repair, params, request_text)
                if cid and not str(params.get("channel_id") or "").isdigit():
                    params["channel_id"] = cid
                    logger.info(f"[AGENT REPAIR] create_automation resolved channel_id {cid} for {params.get('name')}")
                # role_name -> role_id for trigger_role
                if params.get("role_name") and not str(params.get("role_id") or "").isdigit() and guild_for_repair:
                    rn = str(params["role_name"]).lower().strip()
                    for r in getattr(guild_for_repair, "roles", []) or []:
                        if getattr(r, "name", "").lower() == rn:
                            params["role_id"] = str(r.id)
                            logger.info(f"[AGENT REPAIR] resolved role_name {rn} -> {r.id}")
                            break
                # Bulk items channel repair
            if name == "bulk_create_automations":
                guild_for_repair = getattr(interaction, "guild", None)
                items = params.get("automations") or []
                if isinstance(items, list):
                    for it in items:
                        if isinstance(it, dict) and not str(it.get("channel_id") or "").isdigit():
                            cid2 = _resolve_channel_id(guild_for_repair, it, request_text)
                            if cid2:
                                it["channel_id"] = cid2
            qerr = _check_quota_or_error(guild_for_repair, name, params)
            if qerr:
                return Receipt(action=name, success=False, verified=False, error_type=ErrorType.INVALID_PARAMS,
                    message=qerr, request_id=request_id, job_id=job_id, parameters=dict(params),
                    started_at=started, finished_at=time.time())
        except Exception as e:
            logger.debug(f"quota/channel repair skipped: {e}")
