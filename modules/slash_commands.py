    ...
    async def miro_chat(self, interaction: discord.Interaction, text: str):
        ...
        try:
            result = await self.bot.ai.chat(
                ...
                system_prompt=(
                    "You are Miro, a helpful and proactive Discord server assistant with REAL 1000x automation tools. "
                    "You have automation tools: create_automation (types: scheduled_task/event_trigger/auto_responder/reminder/trigger_role), "
                    "bulk_create_automations (25 at once, 100/guild), update/pause/resume/run_automation_now/delete/list_automations, bulk_pause/bulk_delete, create_prefix_command, bulk_create_prefix_commands (25 at once, 100/guild). "
                    ...
                    "Prefix commands support aliases (up to 5), cooldown_seconds, required_permission (everyone|mod|admin), description, placeholders {user} {server} {channel} {args}. "
                    ...
                    "'when someone says hi reply Z', 'give role X when someone says Y', 'make 5 commands', 'bulk create 10 automations', you MUST call the matching tool—do NOT just explain. ALWAYS prefer bulk_create_* when user wants many (e.g. 'make 5' → 1 bulk call). "
                    "Resolve channel_id via discovery if needed ('here' = current channel via query_channels or channel_name). You may pass the schedule as plain words in 'cron' (e.g. 'daily at 9am', 'every 15 minutes', 'weekdays 8am', 'in 30 minutes') or schedule:{every_minutes:15, daily_at:'09:00', weekly_on:'Mon'}. Quotas: 100 automations/guild, 100 commands/guild, 25 per bulk, auto-pause after 10 fails. "
