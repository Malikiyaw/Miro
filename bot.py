...
class MiroBot(commands.Bot):
    def __init__(self, proxy: str = None, proxy_auth=None):
        ...
        self._automations_restored = False
    ...
    async def _start_background_tasks(self):
        ...
        # NOTE: self.guilds is EMPTY in setup_hook (pre-READY), so real restore
        # is deferred to on_ready after Supabase pull. Keep a guarded no-op here
        # for local runs where setup_hook may have guilds cached.
        try:
            if getattr(self, "guilds", None) and len(list(self.guilds)) > 0:
    ...
    async def _auto_backup_loop(self):
        """Automatic data backup every 6 hours (+ Supabase heartbeat)."""
        while True:
            try:
                dm.backup_data()
            ...
            # Supabase heartbeat: push local guilds every 6h even without saves
            try:
                from utils.supabase_sync import is_configured, push_all
                if is_configured():
                    await push_all(dm.data_dir)
            except Exception as e:
    ...
    async def on_ready(self):
        ...
        # Supabase immortal restore: if ./data wiped (Render Free deploy) but
        # Supabase has truth, pull before any rehydrate. Non-blocking, no crash.
        try:
            from utils.supabase_sync import is_configured, pull_all, push_all
            if is_configured():
                import glob as _glob
                local_guilds = _glob.glob(os.path.join(dm.data_dir, "guild_*.json"))
                if not local_guilds:
                    cnt = await pull_all(dm.data_dir)
                    if cnt:
                        logger.info(f"Supabase restore: pulled {cnt} guild(s) after wipe")
                    else:
                        logger.info("Supabase: no remote guilds to restore (first run)")
                else:
                    # Local has months of data but remote may be empty (first enable).
                    # Migrate once: push local to Supabase so future wipes restore.
                    if not getattr(self, "_supabase_migrated", False):
                        self._supabase_migrated = True
                        asyncio.create_task(push_all(dm.data_dir))
                        logger.info("Supabase: migrating local guilds to remote (first enable)")
        except Exception as e:
            logger.warning(f"Supabase on_ready pull failed: {e}")

        # Deferred rehydrate: guilds are NOW populated (post-READY), so cron jobs
        # actually get scheduled. Also handles multi-step + catch-up + full types.
        if not getattr(self, "_automations_restored", False):
            try:
                # _restore_automations is sync (heap ops) – run directly, but
                # wrap to catch croniter errors
                self._restore_automations()
                self._automations_restored = True
            except Exception as e:
                logger.error(f"Deferred automation restore failed: {e}")
    ...
    async def _graceful_shutdown(self):
        ...
        try:
            # Save all data + flush to Supabase (immortal)
            dm.backup_data()
            try:
                from utils.supabase_sync import is_configured, push_all
                if is_configured():
                ...
                logger.debug(f"Supabase shutdown push failed: {e}")
    ...
    def _restore_automations(self):
        ...
        survive dead/offline/restart/redeploy. Handles single + multi-step
        scheduled_task with catch-up (missed <24h -> run once in 60s)."""
        try:
        ...
        for guild in list(self.guilds):
            ...
            for name, entry in list(autos.items()):
                try:
                    ...
                    if entry.get("paused"):
                        continue
                    # only scheduled_task needs task_scheduler; other types (event_trigger,
                    # reminder, auto_responder, trigger_role) are passive and read on message
                    if entry.get("type") != "scheduled_task":
                        continue
                    cron = entry.get("cron")
                    # support both legacy single handler/params and new actions list
                    actions_list = entry.get("actions")
                    sched_handler = entry.get("handler")
                    sched_params = entry.get("params") or {}
                    if isinstance(actions_list, list) and actions_list:
                        # multi-step: keep as is, no single handler needed
                        cron_ok = bool(cron)
                    else:
                        actions_list = None
                        cron_ok = bool(cron and sched_handler)
                    if not cron_ok:
                        continue
                    try:
                        ...
                        nxt = 0.0
                    # catch-up: if missed while dead/offline, run once if within 24h
                    if nxt <= now:
                        if nxt and (now - nxt) < 86400:
                            # missed within a day -> catch-up in 60s, then future
                            nxt = now + 60
                            logger.info(f"Automation '{name}' missed run ({int(now-nxt)}s ago) -> catch-up in 60s")
                        else:
                            try:
                    ...
                    if channel_id is not None and "channel_id" not in sched_params and "channel" not in sched_params:
                        sched_params = dict(sched_params)
                        sched_params["channel_id"] = channel_id

                    # capture per-iteration values to avoid closure bug
                    _actions = list(actions_list) if actions_list else None
                    _cron = cron
                    _h = sched_handler
                    _p = dict(sched_params)
                    _gid = guild.id
                    _name = name

                    async def _run(_name=_name, _cron=_cron, _h=_h, _p=_p, _gid=_gid, _actions=_actions):
                        try:
                            ...
                            if _actions:
                                for step in _actions:
                                    try:
                                        sh = step.get("handler") or step.get("name")
                                        sp = step.get("params") or step.get("parameters") or {}
                                        if not isinstance(sp, dict):
                                            sp = {}
                                        await handler.dispatch(mock, sh, dict(sp))
                                    except Exception as se:
                                        logger.error(f"Automation '{_name}' step {step} failed: {se}")
                            else:
        ...
        if restored:
            logger.info(f"Restored {restored} LIVE cron automation(s) (immortal per guild)")
